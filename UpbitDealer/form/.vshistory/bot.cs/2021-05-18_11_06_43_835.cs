...
namespace UpbitDealer.form
{
    public partial class Bot : Form
    {
        ...
        {
            Algorithm = (Algorithm)cmbAlgorithm.SelectedItem;
            Al
        }
        ...
        private void PointHalfStrategy()
        {
            ...
            try
            {
                if (avgPrice + (2 * (avgPrice * TriggerRate / 100)) <= currPrice && coinPrice >= 5000)
                {
                ...
                else if (ratio >= TriggerRate && coinPrice >= 5000 && ((DateTime.Now - LastSellDate).TotalMinutes >= idx || ratio >= TriggerRate * 1.5))
                {
                    // 올랐을때 코인 금액 절반 팔기
                ...
                else if (ratio <= -(TriggerRate * 1.2) && krwBalance > 5000 && (DateTime.Now - LastBuyDate).TotalMinutes >= idx)
                {
