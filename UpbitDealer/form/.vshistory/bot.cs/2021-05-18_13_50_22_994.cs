    ...
    public partial class Bot : Form
    {
        ...
        private void MinuteCandleByTicks()
        {
            ...
            if (result != null)
            {
                ...
                var krwProfit = StartKRW - StartKRW;
                var krwProfitRate = StartKRW -;
                var profits = new object[] { coinName, StartKRW, chance.KRWBalance + chance.CoinBalance };
                WriteLog("#### RESULT : {0}", result.ToString());
                WriteLog("#### {0} 수익 : 거래시작금액 {1:N0}, 현재평가 금액 {2:N0}, 수익금액 {3:N0}, 수익율 {4F6}", args);
            }
