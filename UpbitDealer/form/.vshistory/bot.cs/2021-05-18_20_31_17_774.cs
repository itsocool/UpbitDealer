...
namespace UpbitDealer.form
{
    public partial class Bot : Form
    {
        ...
        private static double ToOrderPrice(this double source, double krw)
        {
            var orderPrice = Math.Truncate(krw / 1000) * 1000;        // 1000단위 절사
            var rest = krw - orderPrice;

            // 5 단위 처리
            if (krw >= 1_000_000 && krw < 2_000_000)
            {
                orderPrice += Math.Truncate(rest / 500) * 50;
            }
            else if (krw >= 100_000 && krw < 500_000)
            {
        ...
        private double GetOrderPrice(double krw)
        {
            var orderPrice = Math.Truncate(krw / 1000) * 1000;        // 1000단위 절사
            var rest = krw - orderPrice;
