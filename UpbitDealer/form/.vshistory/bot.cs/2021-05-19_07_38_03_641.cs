...
namespace UpbitDealer.form
{
    public partial class Bot : Form
    {
        ...
        private void MinuteCandleByTicks()
        {
            ...
            try
            {
                    ...
                    && currentRate >= (feeRate * 2))
                {
                    ...
                    && (buyTs.TotalSeconds >= CandleType.Minute * 45 && tradeProfitRate <= (feeRate - triggerRate)
                        || tradeProfitRate >= triggerRate + (feeRate * 2)))
                {
                    // SELL
                    // 코인평가금 최소 거래금액 보다 많음
            ...
            if (krwAsset != null)
            {
                krwAmount = krwAsset.Value<double>("balance");
            }
            ...
            var totalKrw = krwAmount + coinAmount;
        ...
        private void GetAccount(string coinName)
        {
            var asset = ApiData.getAsset();
            var krwAsset = asset.Where(x => "KRW".Equals(x.Value<string>("currency"))).FirstOrDefault();
            var coinAsset = asset.Where(x => coinName.Equals(x.Value<string>("currency"))).FirstOrDefault();
