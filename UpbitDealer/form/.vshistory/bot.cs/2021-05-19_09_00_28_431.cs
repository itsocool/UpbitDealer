...
namespace UpbitDealer.form
{
    public partial class Bot : Form
    {
        ...
        private void MinuteCandleByTicks()
        {
            ...
            try
            {
                    ...
                    && currentRate >= (feeRate * 2))
                {
                    ...
                    || tradeProfitRate >= (triggerRate / 2) + (feeRate * 2)))
                {
                    // SELL
                    // 코인평가금 최소 거래금액 보다 많음
        ...
        private Balance GetBalance(ApiData apiData, string coinName, double startKrw)
        {
            ...
            var locked = coinAsset.Value<double>("locked");
            var now = DateTime.Now;

            while(locked > 0 && (DateTime.Now - now).TotalSeconds <= 10)
            {
                asset = apiData.getAsset();
                krwAsset = asset.Where(x => "KRW".Equals(x.Value<string>("currency"))).FirstOrDefault();
                coinAsset = asset.Where(x => coinName.Equals(x.Value<string>("currency")) && "KRW".Equals(x.Value<string>("unit_currency"))).FirstOrDefault();
                locked = coinAsset.Value<double>("locked");
            }
