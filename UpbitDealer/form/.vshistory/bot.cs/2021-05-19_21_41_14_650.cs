...
namespace UpbitDealer.form
{
    public partial class Bot : Form
    {
        ...
        private void MinuteCandleByTicks()
        {
            ...
            var triggerUpPrice = highPrice - (highPrice * triggerRate + (feeRate * 2) / 100);       // 매수 반등 촉발가
            var targetDownPrice = prevPrice + (prevPrice * (feeRate * 2) / 100);                    // 매수 목표가

            WriteLog("triggerUpPrice {0:N0}, targetDownPrice {1:N0}", triggerUpPrice, targetDownPrice);
