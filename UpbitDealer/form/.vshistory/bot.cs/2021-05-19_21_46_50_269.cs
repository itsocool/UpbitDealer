...
namespace UpbitDealer.form
{
    public partial class Bot : Form
    {
        ...
        private void MinuteCandleByTicks()
        {
            ...
            var triggerUpPrice = highPrice + (highPrice * (feeRate * 2) / 100);                     // 매수 반등 촉발가
            //var targetDownPrice = prevPrice + (prevPrice * (feeRate * 2) / 100);                    // 매수 목표가

            WriteLog("currPrice {0:N0}, triggerDownPrice {1:N0}, triggerUpPrice {2:N0}", currPrice, triggerDownPrice, triggerUpPrice);
