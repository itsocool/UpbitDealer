...
namespace UpbitDealer.form
{
    public partial class Bot : Form
    {
        ...
        private void MinuteCandleByTicks()
        {
            ...
            var changePrice = (avgPrice * (((triggerRate / candleCount) + (feeRate * 2))) / 100);   // 수익
            var targetSellPrice = avgPrice + changePrice;                                           // 매도 목표가
            ...
            try
            {
                    ...
                    && downUpRate <= candleCount * 2)                                                               // 반등율이 캔들갯수의 2배수가 넘지 않아야 함
                {
                    ...
                    || currPrice >= targetSellPrice))                                                               // 현재가가 평단가 보다 (수익율/캔들수 + 수료율2배) 이상일때 전체 매도
                {
                    // SELL
                    // 코인평가금 최소 거래금액 보다 많음
