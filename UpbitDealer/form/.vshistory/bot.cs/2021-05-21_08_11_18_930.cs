...
namespace UpbitDealer.form
{
    public partial class Bot : Form
    {
        ...
        private void MinuteCandleByTicks()
        {
            ...
            var buyCoolDown = CandleType.Minute * 60;                                               // 매수 쿨다운 시간 (초)
            var buyCountDownSec = buyCoolDown CandleType.Minute * 60;                                               // 매수 쿨다운 시간 (초)

            var sellTs = (DateTime.Now - LastBuyDate).TotalSeconds;                                 // 매수 경과 시간 (초)
            var sellCoolDown = (CandleType.Minute * 60 * candleCount / 3 * 2);                      // 매도 쿨다운 시간 (초)
            var sellCountDownSec = sellCoolDown - sellTs;                                           // 매도 까지 남은 시간 (초)
