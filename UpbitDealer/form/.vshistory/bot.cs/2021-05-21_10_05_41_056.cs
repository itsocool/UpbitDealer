    ...
    public partial class Bot : Form
    {
        ...
        private void MinuteCandleByTicks()
        {
            ...
            try
            {
                ...
                {
                    // 거래 대기 중일때
                    var lastTradeSide = LastTradeResult.Value<string>("side");
                    var lastTradeState = LastTradeResult.Value<string>("state");
                    WriteLog("#### 거래 불가(거래 대기중) : lastTradeSide {0}, lastTradeState {1}", lastTradeSide, lastTradeState);
                    return;
                }
                    ...
                    && downUpRate <= candleCount * 2)                                                               // 반등율이 캔들갯수의 2배수가 넘지 않아야 함
                {
                    ...
                    LastTradeResult = React.executeDeal(true, false, coinName, 0, 0, total);
                    WriteLog("#### BUY : {0}", LastTradeResult.ToString(Formatting.Indented));
                    ...
                    && currPrice >= Math.Truncate(targetSellPrice))                                                 // 현재가가 목표 매수 금액보다 클때
                {
                    ...
                    vol = Math.Truncate(vol * 100000) / 100000;
                    result = React.executeDeal(false, false, coinName, vol, 0, 0);
                    WriteLog("#### SELL : {0}", LastTradeResult.ToString(Formatting.Indented));
                }
