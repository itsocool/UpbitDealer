...
namespace UpbitDealer.form
{
    public partial class Bot : Form
    {
        ...
        private void MinuteCandleByTicks()
        {
            ...
            line1 += $" : 현재가/직전가/최고가 {currPrice:N0} / {prevPrice:N0} / {highPrice:N0}";
            line1 += $", 하락율/ 반등율 {downRate:F4} / {upRate:F4}, 평단가 {avgPrice:N0}";
            line1 += (sellCountDownSec > 0) ? $", 매수쿨 {sellCountDownSec:N0}(초)" : "";
            line1 += (buyCountDownSec > 0) ? $", 매도쿨 {buyCountDownSec:N0}(초)" : "";
            ...
            try
            {
                ...
                if (krwBalance > minTradeKRW                                                                        // 보유 현금이 최소거래 금액 보다 클때
                    && buyCountDownSec > 0                                                                          // 매도 유예시간이 지났을때
                    ...
                    && downUpRate <= candleCount * 2)                                                               // 반등율이 캔들갯수의 2배수가 넘지 않아야 함
                {
                    ...
                    && (sellCountDownSec < 0 || currPrice >= Math.Truncate(targetSellPrice)))                       // 매수 유예시간이 지나거나 현재가가 목표 매수 금액보다 클때
                {
                    // SELL
                    var vol = coinVol;
                    vol = Math.Truncate(vol * 100000) / 100000;
