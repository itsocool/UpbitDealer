...
namespace UpbitDealer.form
{
    public partial class Bot : Form
    {
        ...
        private async Task MinuteCandleByTicksAsync()
        {
            ...
            try
            {
                    ...
                    && downUpRate <= candleCount * 2)                                                               // 반등율이 캔들갯수의 2배수가 넘지 않아야 함
                {
                    ...
                    if(orderCheck != null)
                    {
                        var price = orderCheck.Value<double>("price");
                        price = GetOrderUnitPrice(price + Math.Max((price * 0.5 / 100) + (fee * 2), 10));
                        var vol = orderCheck.Value<double>("executed_volume");
                        var priceSellResult = await GetSellPriceResultAsync(coinName, price, vol);
                        orderResult = React.executeDeal(false, false, coinName, vol, 0, 0);
                        LastSellDate = DateTime.Now;
                        WriteLog("#### SELL : {0}", orderResult.ToString(Formatting.Indented));
                    }
        ...
        private async Task PointHalfStrategy()
        {
            await Task.Delay(0);
        ...
        private async Task<JObject> GetBuyMarketResultAsync(string uuid)
        {
        ...
        private async Task<JObject> GetSellPriceResultAsync(string coinName, double price, double vol, int timeOutSec = 100)
        {
            await Task.Delay(500);
            ...
            if (orderResult != null)
            {
                ...
                try
                {
                    while (succeeded)
                    {
                        ...
                        {
                            result = apiData.cancelOrder(uuid);
                            await Task.Delay(500);
                            result = apiData.checkOrder(uuid);
                            WriteLog("#### price sell checkOrder : {0}", result.ToString(Formatting.Indented));
                            succeeded = true;
                            break;
                        }
                    }
        ...
        private double ToOrderPrice(double price)
        {
