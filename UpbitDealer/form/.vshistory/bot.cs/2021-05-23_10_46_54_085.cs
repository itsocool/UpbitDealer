            ...
            try
            {
                    ...
                    && downUpRate <= candleCount * 2)                                                               // 반등율이 캔들갯수의 2배수가 넘지 않아야 함
                {
                    ...
                    if(buyOrderResult != null)
                    {
                        ...
                        var sellOrderResult = await GetSellResultAsync(coinName, targetBuyPrice, buyVol);
                        IsWaiting = false;
                        LastBuyDate = DateTime.Now;
                        WriteLog("#### SELL : {0}", sellOrderResult.ToString(Formatting.Indented));

                        if (sellOrderResult != null)
                        {
                            var sellUuid = sellOrderResult.Value<string>("uuid");
                            var sellState = sellOrderResult.Value<string>("state");
                            var sellPrice = order.Value<double?>("price") ?? 0D;
                            var sellVolume = order.Value<double?>("volume") ?? 0D;
                            var tradeCount = order.Value<double>("trade_count");
                            var trades = order["trades"] as JArray;
                            var tradePrice = 0D;
                            var tradeVolume = 0D;
